...
namespace ExpMQManager.BLL
{
    public class GenerateDIS : GenerateBase
    {
        ...
        public string buildUpDIS(DisEntity msgEntity, string msgType, string subType)
        {
            ...
            strAWB += msgEntity.disPrefix + "-" + msgEntity.disAWB + msgEntity.origin + msgEntity.dest + "/T" + msgEntity.manPcs + "/K" + string.Format("{0:0.0}", msgEntity.weightRCF) +  "\r\n";
            strAWB += subType.ToUpper() + "/" + msgEntity.flightNo + "/" + disTime + "/" + msgEntity.destFlight + "/" + msgEntity.disType + "/";

            if(msgEntity.disType == "FDAW" || msgEntity.disType == "MSAW")
            {
            ...
            else if(msgEntity.disType == "MSCA")
            {
                ...
                else
                {
                    ...
                    strAWB += "P" + tempPCs + "K" + string.Format("{0:0.0}", tempWeight);
                }
            }
            ...
            if (msgEntity.disOsi != null && msgEntity.disOsi != string.Empty)
            {
                ...
                {
                    strAWB += msgEntity.disOsi.Substring(0, 65).ToUpper() + "\r\n";
                    strAWB += "/" + msgEntity.disOsi.Substring(65, (msgEntity.disOsi.Length > 130 ? 130 : msgEntity.disOsi.Length)).ToUpper();
                }
                else
                {
                    strAWB += msgEntity.disOsi.ToUpper();
                }
            }
