    ...
    public class GenerateDIS : GenerateBase
    {
        ...
        public string buildUpDIS(DisEntity msgEntity, string msgType, string subType)
        {
            ...
            else if(msgEntity.disType == "MSCA")
            {
                ...
                if(missingPCs < 0)
                    throw new Exception("DIS: Missing Pc is 0. QueueID: " + msgEntity.queueId);

                double tempWeight = (msgEntity.weightRCF / msgEntity.manPcs) * missingPCs;

                strAWB += "P" + msgEntity.locPcs+ "K" + string.Format("{0:0.0}", msgEntity.weightRCF) + "\r\n";
