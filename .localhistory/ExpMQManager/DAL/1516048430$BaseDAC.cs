                    ...
                    case "DIS":
                        strSql = @"
                                    ...
                                    SELECT   A.iid 
                                        ...
                                        ON A.msgType = D.msgType AND A.Carrier = D.Carrier AND D.Active = '1' 
		                                    AND ISNULL(D.Destination, '') = ( CASE WHEN EXISTS (SELECT MsgAddress FROM EDI_Address WHERE Carrier = A.Carrier AND Dest = C.Dest AND msgType = 'FSU' AND (SubMsgType LIKE '%RTF%' or SubMsgType is NULL or SubMsgType = '') AND Active = '1') THEN C.Dest
                                                                                WHEN EXISTS  (SELECT MsgAddress FROM EDI_Address WHERE Carrier = A.Carrier AND (Dest = '' or Dest is NULL) AND msgType = 'FSU' AND (SubMsgType LIKE '%RTF%' or SubMsgType is NULL or SubMsgType = '') AND Active = '1') THEN ''
											                                    ELSE 'NON' END)
		                                    AND (D.subMsgType LIKE '%RTF%' or D.SubMsgType is NULL or D.SubMsgType = '')

	                                    WHERE A.msgType = 'FSU' AND A.subMsgType = '{3}' AND A.iid = @queueID
                                    ";
