    ...
    public class DisDAC : BaseDAC
    {
        public DisEntity GetDISInfoDAC(int mid, int refID, int flightSeq, string msgType, string subType, int queueId)
        {
            ...
            strSql = @"
                        ...
                        JOIN (SELECT idnum, Prefix, AWB, FlightSeq, [Weight], ManPcs, LocPcs, RCF_PCS, DIS_Type, OSI FROM RCF_BCLItem WHERE FlightSeq = @FlightSeq AND MID = @MID) as RCFBCL
                        ON EDIMQ.FlightSeq = RCFBCL.FlightSeq AND EDIMQ.RefID = RCFBCL.idnum
